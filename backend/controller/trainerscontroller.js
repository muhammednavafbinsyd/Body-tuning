...
exports.createTrainers = asyncHandler(async (req, res) => {
  const { firstname, lastname, email, contact, description, status } = req.body;
...
exports.trainersGet = asyncHandler(async (req, res) => {
  console.log("success-trainers get");
  try {
    const elements = await trainersModel.find();
    res.json(elements);
  } catch (err) {
...
exports.trainersEdit = asyncHandler(async (req, res) => {
  const { id } = req.params;
  console.log("trainersedit");
  console.log("trainersedit", id);
  console.log(id);
...
exports.trainersUpdate = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const updateElement = await trainersModel.findByIdAndUpdate(
      id,
      { firstname, lastname, email, contact, description, image, status }, // Include image in the update
      { new: true }
    );
...
exports.deleteTrainers = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (deleteTrainersData) {
      ...
      console.log(deleteTrainersData);
    } else {
