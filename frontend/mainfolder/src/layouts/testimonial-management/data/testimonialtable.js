...
function Data() {
  ...
  const testimonialList = async () => {
    ...
    try {
      const response = await axios.get("http://localhost:2000/adminroute/testimonialList");
  ...
  const remove = async (id) => {
    try {
      await axios.delete(`http://localhost:2000/adminroute/deleteTestimonials/${id}`);
      testimonialList();
      closeDeleteDialog();
    } catch (error) {
      console.log(error);
  ...
  return {
    ...
    rows: list.map((item) => ({
      image: <Image image={`http://localhost:2000/uploads/${item.image}`} />,
