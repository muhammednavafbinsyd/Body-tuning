...
function UserData({ subscribed, page, limit }) {
  ...
  const getsubscribeddata = async () => {
    ...
    try {
      const response = await axios.get("http://localhost:2000/adminroute/subscribedornot");
  ...
  const getusers = async () => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:2000/userroute/getUsers?subscribefilter=${subscribed}&page=${page}&limit=${limit}`
      );
  ...
  const SubscriptionStatus = ({ user }) => {
    ...
    if (userStatuses.length > 0) {
      ...
      if (latestStatus.status === "active") {
        return (
          // <SoftBox display="flex" flexDirection="column">
          //   <Status status={<div key={latestStatus._id}>  {latestStatus.status}  </div>} />
          // </SoftBox>
          <SoftBox display="flex" flexDirection="column">
  ...
  //     if (user && user._id) {
  //       const userTypes = List2.filter((type) => type.userID === user._id);

  //       if (userTypes.length > 0) {
  //         const latestTypes = userTypes[userTypes.length - 1];

  //         return (
  //           <SoftBox display="flex" flexDirection="column">
  //             <Type type={<div key={latestTypes._id}>{latestTypes.type}</div>} />
  //           </SoftBox>
  //         );
  //       } else {
  //         return (
  //           <SoftBox display="flex" flexDirection="column">
  //             <Type type="Not Subscribed" />
  //           </SoftBox>
  //         );
  //       }
  //     } else {
